...
def pil_to_fig(im, showlegend=False, title=None):
    ...
    fig.add_layout_image(dict(
        source=im,  opacity=1, layer="below",
    ...
    fig.update_layout(title=title, showlegend=showlegend,width=1920,height=1080)
...
def show_img(img_path):
    ...
    if os.path.exists(xml_path):
        ...
        for obj in ann_obj:
            ...
            add_bbox(
                fig, x0, y0, x1, y1,
                opacity=0.7, group=label, name=label, color="green",
                showlegend=True, text=label,
            )
